...
export class UserReportComponent {
  ...
  getData() { 
    if (this.filter.dateFrom != null && this.filter.dateFrom !== '') {
      const parsedDateFrom = moment(this.filter.dateFrom);
      this.filter.dateFrom = parsedDateFrom.isValid() ? parsedDateFrom.format('YYYY-MM-DD') : '';
  } else {
      this.filter.dateFrom = '';
  }
  
  if (this.filter.dateTo != null && this.filter.dateTo !== '') {
      const parsedDateTo = moment(this.filter.dateTo);
      this.filter.dateTo = parsedDateTo.isValid() ? parsedDateTo.format('YYYY-MM-DD') : '';
  } else {
      this.filter.dateTo = '';
  }
  
  // Apply the same condition for joiningDate
  if (this.filter.joiningDate != null && this.filter.joiningDate !== '') {
      const parsedJoiningDate = moment(this.filter.joiningDate);
      this.filter.joiningDate = parsedJoiningDate.isValid() ? parsedJoiningDate.format('YYYY-MM-DD') : '';
  } else {
      this.filter.joiningDate = '';
  }
  

    this.api.getUserReport(
      this.filter?.dateFrom,
      this.filter?.dateTo,
      this.filter?.joiningDate ? this.filter?.joiningDate : '',
      this.filter?.userGroup ? this.filter?.userGroup : 0,
