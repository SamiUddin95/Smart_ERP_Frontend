...
@Component({
  selector: 'app-sale-cash-out-list',
  templateUrl: './sale-cash-out-list.component.html',
  styleUrls: ['./sale-cash-out-list.component.css']
})
export class SaleCashOutListComponent {
  constructor(private router: Router, private api: ApiService, private messageService: MessageService, private confirmationService: ConfirmationService) { }
	...
	filter:any={};
	getList() {
		this.api.getAllCashOut().subscribe((res: any) => { 
			this.saleMan = res;
		})
	}
	addTill() {
		this.router.navigate(['cash-out-form']);
	}
	cancel() {
	...
	editUser(id: any) {
		this.router.navigate(['cash-out-form/' + id]);
	}
	deleteUser(user: any) {
