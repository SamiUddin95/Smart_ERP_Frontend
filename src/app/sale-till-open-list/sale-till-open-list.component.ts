...
@Component({
  selector: 'app-sale-till-open-list',
  templateUrl: './sale-till-open-list.component.html',
  styleUrls: ['./sale-till-open-list.component.css']
})
export class SaleTillOpenListComponent {
  constructor(private router: Router, private api: ApiService, private messageService: MessageService, private confirmationService: ConfirmationService) { }
	...
	filter:any={};
	getList() {
		this.api.getAllTillOpen().subscribe((res: any) => { 
			this.saleMan = res;
		})
	}
	addTill() {
		this.router.navigate(['till-open-form']);
	}
	cancel() {
